...
class AdminDashboardController extends Controller
{
    ...
    private function autoCompletePastAppointments(): void
    {
        DB::transaction(function () {
            /*
            * 1. Opłacone wizyty z przeszłości automatycznie oznaczamy jako zakończone.
            */
            ...
            /*
            * 2. Nieopłacone wizyty oczekujące na płatność, których termin minął,
            * automatycznie anulujemy i zwalniamy slot w grafiku.
            */
            $expiredUnpaidAppointments = Appointment::where('status', 'pending_payment')
                ->where('date', '<', now())
                ->where('payment_status', 'unpaid')
                ->get();

            foreach ($expiredUnpaidAppointments as $appointment) {
                $appointment->update([
                    'status' => 'cancelled',
                ]);

                AvailabilitySlot::where('doctor_id', $appointment->doctor_id)
